import { Flex, Box, Text, ActionIcon, Divider, useMantineTheme } from "@mantine/core";
...
export default function AssetsList() {
    ...
    return (
        ...
        >
            <Text fz={"24px"} fw={600} c="#FFF" mb="md" style={{ borderBottom: `2px solid ${BORDER_COLOR}` }}>
                Asset Classes
            </Text>
                    ...
                    {openClass === assetClass.name && (
                        <Box pl="lg" pb="sm">
                            {assetClass.items.map(item => (
                                <Text key={item} size="sm" c="#B0B0B0" py={3} style={{ cursor: 'pointer' }}>
                                    {item}
                                </Text>
                            ))}
                        </Box>
                    )}
                    {index < assetClasses.length - 1 && <Divider color={BORDER_COLOR} my={2} />}
                </Box>
            ))}
        </Box>
    );
