...
export default function AssetClasses({ onAssetSelect }: AssetClassesProps) {
  ...
  const assetClasses = groupedAssets ? transformAssetData(groupedAssets) : [];
  const didSelectInitialAsset = useRef(false);

  useEffect(() => {
    if (
      !didSelectInitialAsset.current &&
      assetClasses.length > 0 &&
      assetClasses[0].items.length > 0
    ) {
      const firstAsset = assetClasses[0].items[0];
      onAssetSelect(firstAsset.id.toString(), firstAsset.leverage);
      didSelectInitialAsset.current = true;
    }
  }, [assetClasses]);
